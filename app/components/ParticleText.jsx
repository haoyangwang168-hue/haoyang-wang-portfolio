...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const render = (now) => {
      ...
      if (isInViewport && !document.hidden && (!reducedMotion || gathering)) {
    ...
    const ensureRenderLoop = () => {
      if (animationFrame === null && isInViewport && !document.hidden) {
        animationFrame = window.requestAnimationFrame(render);
      }
    };
    ...
    const sampleText = async () => {
      ...
      const maxTextWidth = width * clamp(widthRatio, 0.5, 1);
      offCtx.font = font;
      ...
      const resolvedVerticalScale = clamp(verticalScale, 0.75, 1.6);
      offscreen.width = textWidth + padding * 2;
      offscreen.height = Math.ceil(textHeight * resolvedVerticalScale) + padding * 2;
      offCtx.font = font;
      offCtx.textAlign = "left";
      offCtx.textBaseline = "alphabetic";
      offCtx.fillStyle = "#ffffff";
      offCtx.save();
      offCtx.translate(0, padding);
      ...
      const maxParticles = Math.max(900, Math.min(3600, Math.floor((width * height) / 120)));
    ...
    const handleClick = () => {
      if (trigger === "click" || trigger === "hover") startGather(true);
    };
  ...
  }, [text, particleSize, density, color, highlightColor, scatter, gatherDuration, stagger, pointerRepel, repelRadius, idleDrift, trigger, fontSize, fontWeight, fontFamily, widthRatio, verticalScale, glow]);
